...
export async function GET(request: Request) {
  try {
    ...
    if (!client) return NextResponse.json({ ok: true, source: "demo", tickets: [] });

    const { data, error } = await client
      .from("support_ticket")
      .select("id, user_id, category, subject, priority, status, created_at, updated_at")
      .order("created_at", { ascending: false })
      .limit(50);
    if (error) return NextResponse.json({ ok: false, reason: error.message }, { status: 500 });
    return NextResponse.json({ ok: true, source: "supabase", tickets: data ?? [] });
  } catch (error) {
    return NextResponse.json({ ok: false, reason: error instanceof Error ? error.message : "Support queue unavailable." }, { status: 500 });
  }
}
