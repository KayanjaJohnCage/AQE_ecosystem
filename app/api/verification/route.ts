...
export async function POST(request: Request) {
  try {
    ...
    if (!targetUserId) return NextResponse.json({ ok: false, reason: 'Target user ID is required.' }, { status: 400 })
    ...
    if (!result.ok || !result.verification) return NextResponse.json(result, { status: 500 })
    ...
    return NextResponse.json({ ok: true, verification: result.verification, audit: persistedAudit })
  } catch (error) {
    return NextResponse.json(
      {
        ok: false,
        reason: error instanceof Error ? error.message : 'Verification update failed'
      },
      { status: 400 }
    )
  }
