...
import { createSubscription, persistSubscription } from '../../../lib/aqe/subscription'
...
export async function POST(request: Request) {
  try {
    ...
    const tier = body.tier === 'premium' || body.tier === 'vip' ? body.tier : 'basic'
    const amount = Number(body.amount ?? 0)
    const currency = String(body.currency ?? 'USD').trim().toUpperCase()

    if (!Number.isFinite(amount) || amount < 0 || !/^[A-Z]{3}$/.test(currency)) {
      return NextResponse.json({ ok: false, reason: 'A valid subscription amount and currency are required.' }, { status: 400 })
    }

    const result = createSubscription({ userId: identity.userId, tier, amount, currency })
    ...
    return NextResponse.json({ ok: false, reason: error instanceof Error ? error.message : 'Subscription creation failed' }, { status: 400 })
  }
}