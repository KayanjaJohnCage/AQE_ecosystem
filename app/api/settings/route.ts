...
function normalizeSettings(
  ...
  return {
    tierPrices: {
      basic:
        Number(tierPrices.basic) > 0
          ? Number(tierPrices.basic)
          : defaults.tierPrices.basic,
      premium:
        Number(tierPrices.premium) > 0
          ? Number(tierPrices.premium)
          : defaults.tierPrices.premium,
      vip:
        Number(tierPrices.vip) > 0
          ? Number(tierPrices.vip)
          : defaults.tierPrices.vip,
    },
    renewalPrices: {
      basic:
        Number(renewalPrices.basic) > 0
          ? Number(renewalPrices.basic)
          : defaults.renewalPrices.basic,
      premium:
        Number(renewalPrices.premium) > 0
          ? Number(renewalPrices.premium)
          : defaults.renewalPrices.premium,
      vip:
        Number(renewalPrices.vip) > 0
          ? Number(renewalPrices.vip)
          : defaults.renewalPrices.vip,
    },
    walletCurrency: String(
      value.walletCurrency || defaults.walletCurrency,
    ).toUpperCase(),
    qcExchangeRate:
