...
export async function POST(request: Request) {
  try {
    ...
    const upload = await createMediaUploadUrl({ userId: identity.userId, fileName, kind, mimeType, sizeBytes })

    return NextResponse.json({ ...upload, kind, uploadBucket: 'profile-media' }, { status: upload.ok ? 200 : 400 })
  } catch (error) {
    return NextResponse.json(
      {
        ok: false,
        reason: error instanceof Error ? error.message : 'Media upload validation failed'
      },
      { status: 400 }
    )
  }
