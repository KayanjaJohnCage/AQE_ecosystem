...
import { chargeChatQcFromDatabase, chargeChatQcServer } from '../../../../lib/aqe/qc'
...
export async function POST(request: Request) {
  try {
    ...
    if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
      const databaseResult = await chargeChatQcFromDatabase(identity.userId, Number(body.messageCount ?? 1))
    ...
    return NextResponse.json(
      {
        ok: false,
        reason: error instanceof Error ? error.message : 'Unknown QC charge error'
      },
      { status: 400 }
    )
  }
