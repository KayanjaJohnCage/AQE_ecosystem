...
import { createMarketplaceProduct, persistMarketplaceProduct } from '../../../../lib/aqe/marketplace'

export async function POST(request: Request) {
  try {
    const body = await request.json().catch(() => ({}))
    const identity = await resolveMutationUserId(request, typeof body.sellerId === 'string' ? body.sellerId : undefined)
    ...
    if (!/^[A-Z]{3}$/.test(currency) || !Number.isInteger(inventory) || inventory < 0) {
      return NextResponse.json({ ok: false, reason: 'Valid currency and non-negative inventory are required.' }, { status: 400 })
    }

    const result = createMarketplaceProduct({ sellerId: identity.userId, title, price, currency, inventory, status })
