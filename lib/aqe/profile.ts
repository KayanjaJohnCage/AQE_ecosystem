...
export type ProfileRecord = {
  ...
  services?: string[]
  tier: ProfileTier
  verificationStatus: 'unverified' | 'pending' | 'approved' | 'rejected' | 'resubmission_required'
...
export async function persistProfileRecord(profile: ProfileRecord) {
  ...
  try {
    ...
    const { data, error } = await client.from('profiles').upsert(payload, { onConflict: 'user_id' }).select().maybeSingle()

    if (error) {
      return { ok: true, saved: false, source: 'memory', profile, reason: error.message }
    }
    ...
    return {
      ...
      source: 'memory',
      profile,
      reason: error instanceof Error ? error.message : 'Profile persistence failed'
    }
  }
}
...
export async function getProfileByUserId(userId: string) {
  ...
  try {
    ...
    if (error) {
      return { ok: true, found: false, profile: null, source: 'memory', reason: error.message }
    }
    ...
    return {
      ...
      profile: {
        ...
        verificationStatus: (data.verification_status as ProfileRecord['verificationStatus']) ?? 'pending',
...
export function sanitizeProfilePayload(input: Record<string, unknown>) {
  return {
    ...
    services: Array.isArray(input.services) ? input.services.filter((item) => typeof item === 'string') : undefined,
    tier: input.tier === 'premium' || input.tier === 'vip' ? input.tier : 'basic'
  }
}
