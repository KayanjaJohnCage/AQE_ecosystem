import { createAnonSupabaseClient, createServerSupabaseClient } from '../supabaseServer'
...
export function normalizeRole(value?: string | null): AqeRole | null {
  ...
  if (['admin', 'administrator', 'superadmin', 'super-admin'].includes(compact)) return 'admin'
  if (['customer', 'user', 'member', 'client'].includes(compact)) return 'customer'

  return null
}

export function resolveRoleFromClaims(claims: Record<string, unknown> | null | undefined): AqeRole | null {
  ...
  for (const source of possibleSources) {
    if (Array.isArray(source)) {
      for (const item of source) {
        const normalized = normalizeRole(typeof item === 'string' ? item : String(item ?? ''))
        if (normalized) return normalized
      }
    }

    if (source && typeof source === 'object') {
      const nestedRole = resolveRoleFromClaims(source as Record<string, unknown>)
...
export function resolveRoleFromToken(token: string | null | undefined): AqeRole | null {
  if (!token) return null

  const sanitized = token.trim()
  if (!sanitized || sanitized === 'mock-session-token' || sanitized === 'mock-refresh-token') {
    return null
  }
...
export function getSessionFromRequest(request: Request): AqeSession {
  ...
  const bearerToken = authorization.toLowerCase().startsWith('bearer ') ? authorization.slice(7).trim() : authorization.trim()

  const userId = request.headers.get('x-user-id') ?? request.headers.get('user-id') ?? ''
  const email = request.headers.get('x-user-email') ?? request.headers.get('email') ?? ''
  const roleHeader = request.headers.get('x-user-role') ?? request.headers.get('x-role') ?? request.headers.get('role') ?? ''
...
export async function resolveAuthenticatedSession(request: Request): Promise<AqeSession> {
...
export async function resolveMutationUserId(request: Request, requestedUserId?: string) {
  ...
  if (supabaseConfigured) {
    ...
    if (requestedUserId && requestedUserId !== session.userId) {
      return { ok: false as const, reason: 'User identity does not match the authenticated session.' }
    }

    return { ok: true as const, userId: session.userId }
  }

  return { ok: true as const, userId: requestedUserId || session.userId || 'demo-user' }
}

export async function requireAuthenticatedRoleAccess(request: Request, allowedRoles: AqeRole[]) {
  const session = await resolveAuthenticatedSession(request)
  ...
  if (!allowedRoles.includes(session.role)) {
    return { ok: false as const, reason: `Access denied for role ${session.role}.` }
  }

  return { ok: true as const, session }
}

export function requireRoleAccess(request: Request, allowedRoles: AqeRole[]): { ok: boolean; session?: AqeSession; reason?: string } {
...
export function assertRole(role: string | null | undefined, allowed: AqeRole[]): boolean {
  const normalized = normalizeRole(role)
