...
export function validateUpload({
  ...
  if (!allowed.includes(mimeType)) return { ok: false, reason: `Unsupported ${kind} type.` }
  if (sizeBytes > maxBytes) return { ok: false, reason: `${kind} upload exceeds the size limit.` }

  return { ok: true }
}

export function buildSignedStoragePath(userId: string, fileName: string, kind: 'image' | 'video') {
...
export async function createMediaUploadUrl({
  ...
  const { data, error } = await client.storage.from('profile-media').createSignedUploadUrl(objectPath)

  if (error || !data) {
    return { ok: false, reason: error?.message ?? 'Could not create signed upload URL.' }
  }
