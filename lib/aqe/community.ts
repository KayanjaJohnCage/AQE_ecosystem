...
export function validateCommunityText(value: unknown, field: string) {
  ...
  if (text.length > 2000)
    return { ok: false as const, reason: `${field} is too long.` };
  return { ok: true as const, text };
}

export async function persistProfileComment({
  authorId,
  profileId,
  body,
}: {
  ...
  if (!client) {
    return {
      ...
      saved: false,
      source: "memory",
      comment: { authorId, profileId, body },
    };
  }
  ...
  if (error || !data) {
    return {
      ...
      saved: false,
      source: "supabase",
      reason: error?.message ?? "Comment could not be saved.",
    };
  }
...
export async function persistDirectMessage({
  ...
  if (!client) {
    return {
      ...
      saved: false,
      source: "memory",
      message: { senderId, recipientId, body },
    };
  }
  ...
  if (error || !data) {
    return {
      ...
      saved: false,
      source: "supabase",
      reason: error?.message ?? "Message could not be saved.",
    };
  }
